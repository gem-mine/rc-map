...
let loadPromise_
console.log('script_google', $script_, window.$script_)
  ...
  loadPromise_ = new Promise((resolve, reject) => {
    ...
    if (process.env.NODE_ENV !== 'production') {
      if (Object.keys(bootstrapURLKeys).indexOf('callback') > -1) {
        console.error(
          '"callback" key in bootstrapURLKeys is not allowed, ' + 'use onGoogleApiLoaded property instead' // eslint-disable-line
        )
        throw new Error(
          '"callback" key in bootstrapURLKeys is not allowed, ' + 'use onGoogleApiLoaded property instead'
        )
      }
    }
    ...
    const url =
      bootstrapURLKeys.region && bootstrapURLKeys.region.toLowerCase() === 'cn'
        ? 'http://maps.google.cn'
